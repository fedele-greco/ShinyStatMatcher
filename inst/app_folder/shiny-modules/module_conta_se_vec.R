...
conta_se_vec <- function(df, variables, value, type_out = "Numeric"){
  nlvls <- length(value)
  out <- numeric(nrow(df))
  for(k in 1:nlvls){
...
conta_se_vec_SERVER <- function(id, x){
  ...
  moduleServer(id, function(input, output, session) {
    ...
    x_ric <- reactive({
      req(input$quale_val)
      conta_se_vec(df = x(), variables = names(x()), value = input$quale_val, type_out = input$type)
    })
    ...
    output$tab_check <- DT::renderDT({
      ...
      #if(shiny::isTruthy(input$new_name)) {names(data)[1] <- input$new_name}
